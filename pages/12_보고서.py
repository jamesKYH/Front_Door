...
if not filtered_df.empty:
    ...
    with col3:
        st.metric(label="최고 매출 시간대", value=f"{peak_hour}시대")
    with col4:
        st.metric(label="최고 매출 요일", value=f"{peak_day}요일")
    ...
    st.markdown(f"- **특정 요일 회원 이벤트**: {day_mapping[str(peak_day).zfill(2)]}에 회원 전용 이벤트 개최.")
...
def generate_pdf(title, insights, charts):
    ...
    # 주요 인사이트 추가
    pdf.set_font("Arial", size=12)
    pdf.multi_cell(0, 10, txt="주요 인사이트:")
